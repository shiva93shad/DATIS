...
def getGraphletFreq(signList):
    orbits = [2, 3, 5, 7, 8, 9, 12, 14, 17, 18, 23, 25, 27, 33, 34, 35, 39, 44, 45, 50, 52, 55, 56, 61, 62, 65, 69, 70,
              72]
...
def computeRGFDistances(graphletCounts, outputName):
    ...
    for i in range(len(graphletCounts) - 1):
        ...
        if pairCount % 1000 == 0:
            print
            'RGF Comparison Jobs submitted: ', str(float(pairCount) / jobCount * 100), '%'
    ...
    while finishedCount < pairCount:
        ...
        if finishedCount % 1000 == 0:
            print
            'RGF Distance comparisons finished: ', str(float(finishedCount) / jobCount * 100), '%'
...
class FinishedConsumer(multiprocessing.Process):
    ...
    def run(self):
        ...
        while finishedCount < self.jobCount:
            ...
            if finishedCount % 1000 == 0:
                print
                'Computation completion: ', str(float(finishedCount) / self.jobCount * 100), '%'
...
def computeNetworkProperties(allIndexes, mode):
    ...
    for index in allIndexes:
        ...
        if submitCount % 100 == 0:
            print
            'Network Property Jobs Submitted:', str(float(submitCount) / jobCount * 100), '%'
    ...
    while finishedCount < len(allIndexes):
        ...
        if finishedCount % 100 == 0:
            print
            'Ready Network Properties:', str(float(finishedCount) / jobCount * 100), '%'
...
def compareNetworkProps(propertyList, distanceMode, outputNames):
    ...
    for i in range(len(propertyList) - 1):
        ...
        for j in range(i + 1, len(propertyList)):
            ...
            if pairCount % 1000 == 0:
                print
                'Jobs submitted:', str(float(pairCount) / jobCount * 100), '%'
    ...
    while finishedCount < pairCount:
        ...
        if finishedCount % 1000 == 0:
            print
            'Finished comparisons: ', str(float(finishedCount) / jobCount * 100), '%'
...
def getSpectralSignatures(allIndexes):
    ...
    for index in allIndexes:
        ...
        if submitCount % 100 == 0:
            print
            'Spectrum Computation Jobs Submitted:', str(float(submitCount) / jobCount * 100), '%'
    ...
    while finishedCount < len(allIndexes):
        ...
        if finishedCount % 100 == 0:
            print
            'Ready Spectrums:', str(float(finishedCount) / jobCount * 100), '%'
...
def checkInput(ndumpFolder, distType, num_processes):
    if not os.path.isdir(ndumpFolder):
        print
        'ERROR: Provided network folder path is not found: {0}'.formaT(ndumpFolder)
        exit(0)

    if num_processes < 1:
        print
        'ERROR: Enter a correct number for the number of processes...'
        exit(0)
    ...
    if distType == 'rgf':
        ...
        print
        'ERROR: Unknown network distance type: {0}'.format(distType)
        print
...
if __name__ == "__main__":
    ...
    # Compute the degree based distance measures
        ...
        compareNetworkProps(degreeDists, 1,
                            [os.path.join(ndumpFolder, 'degree_dists.txt'), os.path.join(ndumpFolder, 'av_degree.txt')])

...
def read_networkCSV(networkFile):
	nodeList = []
	edgeList = []
	count=0
	with open(networkFile, "r") as file1:
		for line in file1.readlines():
			if count == 0:
				count += 1
				continue
			nodeList.append(line.strip().split(',')[0])
			nodeList.append(line.strip().split(',')[1])
			edgeList.append([line.strip().split(',')[0],line.strip().split(',')[1]])
			#between = float(line.strip().split(',')[4])
...
if __name__ == "__main__":
	#netFileName = '../dataset/BitcoinAlpha/graphlet/bitcoinalpha.csv'
	...
	(nodeList, edgeList) = read_networkCSV(netFileName)
